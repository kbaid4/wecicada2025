...
// Dummy data for Event Furniture Rental
const furnitureRentalData = [
  { 
    name: 'Luxury Lounge Rentals', 
    location: 'Downtown', 
    rating: 4.8, 
    image: '/images/venues/14.png'
  },
  { 
    name: 'Event Furniture Pros', 
    location: 'City Center', 
    rating: 4.7, 
    image: '/images/venues/14.png'
  },
  { 
    name: 'Chic Seating Co.', 
    location: 'Uptown', 
    rating: 4.9, 
    image: '/images/venues/14.png'
  },
...
const EventFurnitureRentalPage = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const navigate = useNavigate();

  // Filtering and sorting logic
  let filteredServices = furnitureRentalData.filter(service =>
    ...
    <div className={styles['app-container']}>
      ...
      <div className={styles['hotels-toolbar']}>
        <input
          ...
          type="text"
          placeholder="Search event furniture rentals..."
          value={search}
      ...
      <div className={styles['hotels-grid']}>
        {filteredServices.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            No event furniture rental services found.
          </div>
        ) : (
...
export default EventFurnitureRentalPage;
