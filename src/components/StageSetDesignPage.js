...
// Dummy data for Stage & Set Design Services
const stageDesignData = [
  { 
    name: 'Grand Stage Designs', 
...
const StageSetDesignPage = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const navigate = useNavigate();

  // Filtering and sorting logic
  let filteredServices = stageDesignData.filter(service =>
    ...
    <div className={styles['app-container']}>
      ...
      <div className={styles['hotels-toolbar']}>
        <input
          ...
          type="text"
          placeholder="Search stage & set design services..."
          value={search}
      ...
      <div className={styles['hotels-grid']}>
        {filteredServices.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            No stage & set design services found.
          </div>
        ) : (
...
export default StageSetDesignPage;
