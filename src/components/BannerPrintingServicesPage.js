...
// Dummy data for Banner Printing Services
const bannerPrintingData = [
  { 
    name: 'Print Masters', 
    location: 'Downtown', 
    rating: 4.7, 
...
const BannerPrintingServicesPage = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const navigate = useNavigate();

  // Filtering and sorting logic
  let filteredServices = bannerPrintingData.filter(service =>
    ...
    <div className={styles['app-container']}>
      ...
      <div className={styles['hotels-toolbar']}>
        <input
          ...
          type="text"
          placeholder="Search banner printing services..."
          value={search}
      ...
      <div className={styles['hotels-grid']}>
        {filteredServices.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            No banner printing services found.
          </div>
        ) : (
...
export default BannerPrintingServicesPage;
