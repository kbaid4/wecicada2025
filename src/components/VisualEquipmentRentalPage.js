...
// Dummy data for Visual Equipment Rental
const visualEquipmentData = [
  { 
    name: 'Crystal Clear Displays', 
    location: 'Downtown', 
    rating: 4.8, 
    image: '/images/venues/2.png',
  },
  { 
    name: 'ProVision Screens', 
...
const VisualEquipmentRentalPage = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const navigate = useNavigate();

  // Filtering and sorting logic
  let filteredEquipment = visualEquipmentData.filter(equipment =>
    ...
    <div className={styles['app-container']}>
      ...
      <div className={styles['hotels-toolbar']}>
        <input
          ...
          type="text"
          placeholder="Search visual equipment providers..."
          value={search}
      ...
      <div className={styles['hotels-grid']}>
        {filteredEquipment.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            No visual equipment providers found.
          </div>
        ) : (
...
export default VisualEquipmentRentalPage;
