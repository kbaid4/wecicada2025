...
// Dummy data for Lighting Equipment Rental
const lightingEquipmentData = [
  { 
    name: 'Lumen Masters', 
    location: 'Downtown', 
    rating: 4.8, 
    image: '/images/venues/3.png',
  },
  { 
    name: 'Spotlight Pros', 
...
const LightingEquipmentRentalPage = () => {
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const navigate = useNavigate();

  // Filtering and sorting logic
  let filteredEquipment = lightingEquipmentData.filter(equipment =>
    ...
    <div className={styles['app-container']}>
      ...
      <div className={styles['hotels-toolbar']}>
        <input
          ...
          type="text"
          placeholder="Search lighting equipment providers..."
          value={search}
      ...
      <div className={styles['hotels-grid']}>
        {filteredEquipment.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            No lighting equipment providers found.
          </div>
        ) : (
...
export default LightingEquipmentRentalPage;
